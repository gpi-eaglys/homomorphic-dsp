"""
Utilities for exporting a trained MLP to a flat CKKS-compatible JSON.
...
def export_model(dpath_mdl: str, feat_root: str, esc50_metafile: str, esc10: bool, mlp_cls) -> None:
    ...
    feat_stem = meta.get("feat") or os.path.basename(dpath_mdl).removeprefix("mlp-")
    ...
    assert ds.mean is not None and ds.std is not None

    model = mlp_cls(input_dim=meta["input_dim"], hidden=meta["hidden"], num_classes=len(meta["classes"]))
...
def export_all(mdl_root: str, feat_root: str, esc50_metafile: str, esc10: bool, mlp_cls) -> None:
    ...
    for dpath_mdl in mdl_dirs:
        ...
        export_model(dpath_mdl, feat_root, esc50_metafile, esc10, mlp_cls)
